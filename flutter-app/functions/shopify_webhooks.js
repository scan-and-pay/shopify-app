...
exports.appUninstalled = functions
  ...
  try {
    ...
    if (!verifyShopifyWebhook(rawBody, hmacHeader)) {
      console.error('HMAC verification failed for app/uninstalled');
      return res.status(401).send('Unauthorized - Invalid HMAC');
    }

    console.log('HMAC verification passed for app/uninstalled');

    // CRITICAL: Send 200 OK immediately for Shopify's automated check
    // This ensures Shopify always sees success for valid HMAC
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Error processing app/uninstalled webhook asynchronously:', error);
      }
    });
  } catch (error) {
...
exports.customersDataRequest = functions
  ...
  try {
    ...
    console.log('HMAC verification passed for customers/data_request');
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Error processing customers/data_request webhook asynchronously:', error);
      }
    });
  } catch (error) {
    console.error('Error in customers/data_request webhook handler:', error);
    return res.status(500).send('Internal Server Error');
  }
});
...
exports.customersRedact = functions
  ...
  try {
    ...
    console.log('HMAC verification passed for customers/redact');
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Error processing customers/redact webhook asynchronously:', error);
      }
    });
  } catch (error) {
    console.error('Error in customers/redact webhook handler:', error);
    return res.status(500).send('Internal Server Error');
  }
});
...
exports.shopRedact = functions
  ...
  try {
    ...
    console.log('HMAC verification passed for shop/redact');
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Error processing shop/redact webhook asynchronously:', error);
      }
    });
  } catch (error) {
    console.error('Error in shop/redact webhook handler:', error);
    return res.status(500).send('Internal Server Error');
  }
});
